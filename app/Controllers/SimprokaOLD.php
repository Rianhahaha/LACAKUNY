...
class SimprokaOLD extends BaseController
{
    ...
    //     d(ceil(floatval($i/3)));
    // }
    d($this->myTime,$this->bulan);
        $userQuery=$this->UsersModel->getUsers();
        // $paramsCB=[
        //     'tahun'     => $this->tahun,
        //     'kode'      => 'DK.4470'
        // ];
        // $queryCB=$this->SimprokaModel->getAll('simproka',$paramsCB);
        // dd($queryCB->getResultArray());
        $params=[
            'tahun'     => $this->tahun,
            'sistem'    => $this->bulan
        ];
        $fileQuery=$this->SimprokaModel->getAll('file_simproka',$params);
        $data=[
            'id_pengguna'   => $this->id_pengguna,
            'ket_pengguna'  => $this->ket_pengguna,
            'file'          => $fileQuery->getResultArray(),
        ...
        $paramsOTHER['detail']='OTHER';
        $queryOTHER=$this->SimprokaModel->getAll('simproka',$paramsOTHER);
        $allSimproka=array_merge($query->getResultArray(),$queryOTHER->getResultArray());
        asort($allSimproka);
        $data['data']=$allSimproka;
        ...
        if($this->detail=='RO'){
            ...
            // ];
            $dataFilter=[];
            $params['detail']='KOMPONEN';
            $queryKomponen=$this->SimprokaModel->getAll('simproka',$params);
            $params['detail']='SUBKOMPONEN';
            $querySubkomponen=$this->SimprokaModel->getAll('simproka',$params);
            $allData=array_merge($query->getResultArray(),$queryKomponen->getResultArray(),$querySubkomponen->getResultArray());
            asort($allData);
            if(null!=$this->session->get('kode')){
                $kode=$this->session->get('kode');
                $spliteKode=explode('.',$kode);
                foreach ($allData as $ad => $value) {
                    $kodeSP=explode('.',trim($value['kode']));
                    if(($spliteKode[1]==$kodeSP[1])&&($spliteKode[2]==$kodeSP[2])){
                        $dataFilter[]=$value;
                        if ($value['status_validasi']==null) {
                            $lastData=$value['kode'];
                        }
                        $trueLast=$value['kode'];
                    }
                ...
                if(trim($lastData)==trim($trueLast)){
                    $paramsX=[
                        'tahun'     => $this->tahun,
                        'sistem'    => $this->bulan,
                        'kode'      => $lastData
                    ];
                    $valX['status_validasi']='belum mengajukan';
                    $this->SimprokaModel->updateAll('simproka',$paramsX, $valX);
                    return  redirect()->to(base_url('/simprokaOLD'))->withInput();
                }
                $data['kode']=$kode;
                d($data);
                return view('pages/simprokaOLD',$data); 
            }
            $data['kode']=$kode;
            $data['kodeSebelum']='dummy';
            d($data,$kode);
           
            return view('pages/simprokaOLD',$data);
        }
        // else if($this->detail=='KRO'){
        //     // $data=[
        //     //     'id_pengguna'   => $this->id_pengguna,
        //     //     'ket_pengguna'  => $this->ket_pengguna,
        //     //     'tahun'         => $this->tahun,
        ...
        //     $dataFilter=[];
        //     $params['detail']='RO';
        //     $queryRO=$this->SimprokaModel->getAll('simproka',$params);
        //     $dataRO=$queryRO->getResultArray();
        //     $dataKRO=$query->getResultArray();
        //     // dd($dataRO, $dataKRO);
        //     foreach ($dataKRO as $dKRO => $vl) {
        //         $kode=$vl['kode'];
        //         $spliteKode=explode('.',trim($kode));
        //         foreach ($dataRO as $dRO => $value) {
        //             $kodeSP=explode('.',trim($value['kode']));
        //             if(($spliteKode[1]==$kodeSP[1])&&($spliteKode[2]==$kodeSP[2])){
        //                 $dataFilter[]=$value;
        //                 if ($value['status_validasi']==null) {
    ...
    public function readExcel()
    {
        ...
        return redirect()->to(base_url('/simprokaOLD'));
    }
